...
for i in range(0,2):
    ...
    figs[i*5+3].suptitle("Validation accuracy versus time and Inference accuracy ({})".format(b), y=0.94)
...
for n in range(0, 2):
    ...
    for i in range(len(backends)):
        ...
        axes[n*5+0].plot(train_cost_epoch['{}_t'.format(b)], train_cost_epoch['{}_loss'.format(b)],\
        ...
        axes[n*5+1][int(i/3)][i%3].legend(loc=4)
        ...
        axes[n*5+2].plot(train_acc_epoch['{}_t'.format(b)], -1*train_acc_epoch['{}_acc'.format(b)]+101,\
            marker=markers[b], color=colors[b])
        
        # Valid acc per epoch
        axes[n*5+3].plot(train_acc_epoch['{}_t'.format(b)], -1*valid_acc_epoch['{}_acc'.format(b)]+101,\
            marker=markers[b], color=colors[b])
        ...
        axes[n*5+4][int(i/3)][i%3].legend(loc=4)
    ...
    axes[n*5+0].legend(loc=4)
    axes[n*5+0].set_ylim((-0.1,ylim_high[model]))
    ...
    axes[n*5+2].legend(handles, [label[0:label.rfind('_')] for label in labels], loc=4)
    axes[n*5+2].axvline(fastest[1], linestyle='dashed', color='#777777')
    axes[n*5+2].text(fastest[1]*1.01, 12, "First Finished Training: " + fastest[0], size=12)
    ...
    axes[n*5+2].set_ylabel('Training Accuracy (%)')

    handles, labels = axes[n*5+3].get_legend_handles_labels()
    axes[n*5+3].legend(handles, [label[0:label.rfind('_')] for label in labels], loc=4)
    axes[n*5+3].axvline(fastest[1], linestyle='dashed', color='#777777')
    axes[n*5+3].text(fastest[1]*1.01, 12, "First Finished Training: " + fastest[0], size=12)
    ...
    axes[n*5+3].set_ylabel('Validation Accuracy (%)')

    figs[n*5+0].savefig(root+"/pics/{}/{}_train_loss_versus_time.png".format(model, device), dpi=figs[n*5+0].dpi)
    figs[n*5+1].savefig(root+"/pics/{}/{}_train_and_valid_loss_versus_epoch.png".format(model, device), dpi=figs[n*5+1].dpi)
    figs[n*5+2].savefig(root+"/pics/{}/{}_train_acc_versus_time.png".format(model, device), dpi=figs[n*5+2].dpi)
    figs[n*5+3].savefig(root+"/pics/{}/{}_valid_acc_versus_time.png".format(model, device), dpi=figs[n*5+3].dpi)
    figs[n*5+4].savefig(root+"/pics/{}/{}_train_and_valid_acc_versus_epoch.png".format(model, device), dpi=figs[n*5+4].dpi)
